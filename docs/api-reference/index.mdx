...
either direction: `@AckType()` generates a model from a schema, and
...
`.ack.dart` and `.g.dart` part directives, run:
...
**Generates**: A public `<ClassName>Schema` facade backed by a private codec,
top-level JSON/runtime bridges, and `toJson` / `safeToJson` extension methods
...
Use `@AckField(schema: functionName)` with a top-level schema-function tear-off
for a custom type or `Map<String, V>` field. A sealed base annotated with
`@AckModel(discriminatorKey: ...)` generates a discriminated union from its
same-library concrete branches.
...
The facade exposes `schema`, `parse`, `safeParse`, `fromJson`, `encode`,
`safeEncode`, `toJsonSchema`, and `toSchemaModel`. The backing `_accountSchema`
is library-private and no public lower-camel alias is generated. Set
`schemaName: 'WireAccountSchema'` to choose the exact facade class name.
...
schema-first `@AckType()` declaration may also compose the facade explicitly,
...
unsupported; use schema-first `@AckType()` and named `Ack.lazy` for those
cases. See
