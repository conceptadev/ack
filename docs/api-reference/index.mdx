...
**Generates**: An Ack codec schema, top-level JSON/runtime bridges, and
`toJson` / `safeToJson` extension methods
...
value roots and undiscriminated `anyOf` shapes are unsupported; use
schema-first `@AckType()` for those cases. See
