...
void processApiResponse(String jsonString) {
  ...
  if (result.isOk) {
    ...
    // or use an AckType-generated immutable model (see next section).
  } else {
...
if (result.isOk) {
  ...
  // - AckType model: User.parse(validData)
...
Once a schema is annotated with `@AckType()` (see [TypeSafe Schemas](./typesafe-schemas.mdx) for setup), parse JSON into a generated immutable model. Public `fromJson` / `toJson` still cross the source Ack schema; `json_serializable` only maps the already-validated runtime value onto stored fields.
...
- **Model conversion:** After validation, how you convert the validated map into an app model is up to you. `@AckType()` models still parse and encode through the source schema; `json_serializable` only maps that runtime value onto stored fields.
