...
- `ProfileSchema.schema` returns the real `AckSchema` for composition.

Generated `toJson()` and `safeToJson()` extensions delegate through that same
...
Class-first fields may also use a model generated by `@AckType()`; Ack composes
