...
`safeToJson()`, a generated `copyWith` where omission keeps the current value and
explicit `null` clears a nullable field, and deep collection-aware `==`,
`hashCode`, and `toString`. A sealed
