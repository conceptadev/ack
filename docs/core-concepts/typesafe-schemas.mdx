...
| --- | --- | --- |
| An Ack schema | `@AckType()` | An immutable Dart model |
...
The two code-generation annotations are `@AckType()` and `@AckModel()`.
...
Ordinary `@JsonSerializable()` classes may also use the same `.g.dart` file,
but do not put `@AckModel()` and `@JsonSerializable()` on the same class.

## Schema-first with `@AckType()`
...
added. Use `@AckType(name: 'AppUser')` when you need an exact class name.
...
Each branch must be a named `@AckType()` object schema in the same library:
...
`profileSchema` validates before constructing `Profile`. `toJson()` and
`safeToJson()` are generated extension methods and validate while encoding.
If an API expects `Profile.fromJson`, add the one-line factory shown in the
working example; Ack does not inject constructors into a hand-written class.
...
| Constructor default | Schema applies `.withDefault(...)` | Encodes the stored value |
...
Use `@AckField` when inference is not enough. Its argument is a const tear-off
of a top-level function returning an `AckSchema`:
...
constructor. Unknown keys are stored there. Encoding writes extras first, so a
declared field or discriminator cannot be replaced by an extra value.

## Which direction should you choose?

Choose `@AckType()` when you want to design the boundary schema first, generate
...
4. Choose `@AckType()` for schema-first or `@AckModel()` for class-first.
