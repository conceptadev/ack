...
Ordinary `@JsonSerializable()` classes share the same `.g.dart` file, because
both generators write shared parts. Do not put `@AckModel()` and
`@JsonSerializable()` on the same class.
