...
## Primitive inputs and numeric normalization

String and boolean schemas require the expected Dart runtime type. Numeric
schemas require a `num`, but normalize by mathematical value so native and web
builds behave identically. Non-numeric inputs surface as a `TypeMismatchError`
rather than being silently coerced.

| Schema          | Accepted boundary value                         | Runtime value                        |
| --------------- | ----------------------------------------------- | ------------------------------------ |
| `Ack.string()`  | `String`                                        | unchanged `String`                   |
| `Ack.integer()` | finite `num` with no fractional part, lossless  | normalized `int`                     |
| `Ack.double()`  | finite `num`, exactly representable as `double` | normalized `double`                  |
| `Ack.number()`  | finite `num`                                    | original `int` or `double`           |
| `Ack.boolean()` | `bool`                                          | unchanged `bool`                     |
...
intSchema.parse(42);       // => 42 (int)
intSchema.parse(42.0);     // => 42 (int)
intSchema.safeParse(42.5); // ❌ FAIL: not a mathematical integer
intSchema.safeParse('42'); // ❌ FAIL: TypeMismatchError

final doubleSchema = Ack.double();
doubleSchema.parse(3.14); // => 3.14 (double)
doubleSchema.parse(42);   // => 42.0 (double)
...
`Ack.integer()` and `Ack.double()` overlap for integral values. `Ack.anyOf`
returns the first successful branch, so order numeric branches according to the
runtime representation you want. Use `Ack.number()` when validation should
preserve an input that is already an `int` or `double`. Use `transform`/`codec`
when the boundary value is not already a `num` (for example, a numeric string).

Primitive type checking still lets unions distinguish the string `"42"` from
the numeric value `42` without configuration:
