...
To generate immutable models for hand-written schemas, add the annotation and generator packages alongside `ack`:
...
schema variables and getters annotated with `@AckType()` and emits immutable
models with typed fields, parsing helpers, and JSON methods.
