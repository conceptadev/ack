...
The generator works in both directions. `@AckType()` marks a top-level schema
and generates an immutable model. `@AckModel()` marks a hand-written class and
generates its Ack codec schema and JSON helpers. There is no `@AckSchema()`
annotation; `AckSchema` is the runtime schema type.
...
See [Model Code Generation](../core-concepts/typesafe-schemas.mdx) for working
...
- Dart SDK: `>=3.8.0 <4.0.0` for core `ack`
