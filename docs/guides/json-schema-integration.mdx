...
> **Exporting for a library that doesn't depend on Ack?** Use the vendor-neutral
> [Standard Schema](./standard-schema.mdx) contract instead, so the consumer can
> accept schemas from any participating validator.
